...
/**
 * 通用yt-dlp命令构建器（复用模版逻辑：自动加载Cookie、UA、Referer、超时）
 * @param url 视频链接
 * @param outputPath 输出路径
 * @param extraArgs 额外yt-dlp参数
 */
function buildYtDlpCmd(url: string, outputPath: string, extraArgs = ""): string {
  let baseCmd = `yt-dlp "${url}" -o "${outputPath}" --playlist-items 1 --force-overwrites --no-warnings `
    + `--user-agent "${USER_AGENT}" --referer "${BILIBILI_REFERER}" --socket-timeout 15 ${extraArgs}`;

  // 存在Cookie文件则自动追加，和模版逻辑保持一致
...
export async function downloadAudio(bvid: string, name: string) {
  ...
  const url = `https://www.bilibili.com/video/${bvid}`;
  // 音频专属参数：-x --audio-format mp3
  const audioArgs = `-x --audio-format mp3`;
  const cmd = buildYtDlpCmd(url, output, audioArgs);
