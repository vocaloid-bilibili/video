...
export function getClipSetting(bvid: string) {
  const db = loadClipsDB();
  return db[bvid] || null;
}

export function setClipSetting(bvid: string, startTime: number, endTime: number | null = null) {
  const db = loadClipsDB();
...
export function deleteClipSetting(bvid: string) {
  const db = loadClipsDB();
  if (db[bvid]) {
    delete db[bvid];
    clipsCache = db;
